		...
		for m in mention:
			...
			for r in roles:
				if r.name=="Studying/Working" or r.id==816873155246817290:
					return await message.channel.send(f"{author.mention} Don't ping {m.nick or f'{m.name}# {m.discriminator}'},{m.nick or f'{m.name}# {m.discriminator}'} is Studying/Working")
